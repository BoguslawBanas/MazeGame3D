...
void UpdateCameraCustom(Camera *camera, int mode, Labirynth *labirynth)
{
    Vector2 mousePositionDelta = GetMouseDelta();
    ...
    else
    {
        ...
        Camera prev_c=*camera;

        // Keyboard support
        if (IsKeyDown(KEY_W)) {
            CameraMoveForward(camera, cameraMoveSpeed, moveInWorldPlane);
            if(labirynth->labirynth[int(camera->position.z)][int(camera->position.x)]=='#' && camera->position.y<=5.5f){
                CameraMoveForward(camera, -cameraMoveSpeed, moveInWorldPlane);
            }
        }
        if (IsKeyDown(KEY_A)){
